---
Título: Administrador de Paquetes (pix) - KOOMPI OS
Descripción: Descubra cómo usar pix, el administrador de paquetes de KOOMPI OS.
---

# Administrador de paquetes (`pix`)
...
`pix` introduce completamente un nuevo paradigma en la administración de paquetes. La idea es 
...
Esto se logra instalando el software dentro de uno o más contenedores totalmente administrados por `pix` teniendo acceso restringido a los recursos de su sistema pero con la posibilidad de usar solo sus controladores, servidor gráfico, etc.
...
Por defecto, `pix` proporciona un contenedor basado en la distribución Ubuntu 22.10
para proveer KOOMPI OS 22.10, y aísla todos los comandos del administrador de
...
El nombre `pix` proviene de **apt (Advanced Packaging Tool)**, el administrador de paquetes usado por Debian y sus derivadas. **X** consta de dos líneas (host y contenedor) superpuestas una a otra, donde el contenedor está en la parte superior,
lo que significa que es la parte principal del sistema anfitrión.

## Uso

- [Manual](/docs/pix/manpage)
